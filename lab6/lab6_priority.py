...
def main():
    ...
    for t in range(time_steps):
        ...
        if job.high_priority():
                high_priority_queue.enqueue(job)  # Enqueue the job into high-priority queue
                print(f"Job {job.process_name()} added to HIGH priority queue\n")
        else:
                low_priority_queue.enqueue(job)  # Enqueue yhr job to  low-priority queue
                print(f"Job {job.process_name()} added to LOW priority queue\n")
        ...
        if not process_running:
            if not high_priority_queue.isEmpty():
                current_job = high_priority_queue.dequeue()  # Dequeue high-priority job
                process_running = True  # Set process running to True
                print(f"Running high priority job: {current_job.process_name()}\n")
            elif not low_priority_queue.isEmpty():
                current_job = low_priority_queue.dequeue()  # Dequeue the low-priority job
                process_running = True  # Set process running to True
                print(f"Running low priority job: {current_job.process_name()}\n")
